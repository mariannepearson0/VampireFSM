...
import vampirefsm.journeys.{
  VampireFSMJourneyModel,
  VampireFSMJourneyStateFormats
}
import vampirefsm.repository.{SessionCache, SessionCacheRepository}

import scala.concurrent.{ExecutionContext, Future}

@ImplementedBy(classOf[MongoDBCachedVampireFSMJourneyService])
trait VampireFSMJourneyService extends PersistentJourneyService[HeaderCarrier] {

  val journeyKey = "agentInvitationJourney"

  override val model = VampireFSMJourneyModel
}

@Singleton
class MongoDBCachedVampireFSMJourneyService @Inject()(
    _cacheRepository: SessionCacheRepository)
    extends VampireFSMJourneyService {

  case class PersistentState(state: model.State, breadcrumbs: List[model.State])

  implicit val formats1: Format[model.State] =
    VampireFSMJourneyStateFormats.formats
  ...
  override def clear(implicit hc: HeaderCarrier,
